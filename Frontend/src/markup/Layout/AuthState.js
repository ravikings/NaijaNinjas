...
function AuthState() {
  ...
  const { headerLoading, currentUser: userDetails } = useSelector(
    ...
    const getOnlineState = async () => {
      ...
      if (userID) {
        try {
          ...
          const status = data.user_set_status === false ? true : false
          isMounted && setServerState(data.status)
          isMounted && setOnlineState(status)
          isMounted && setUserProfile(data)
        } catch (error) {
          console.log(error)
        }
      }
